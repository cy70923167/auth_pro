    ...
    <el-dialog v-model="editDialog.visible" title="修改授权目标" width="420px" destroy-on-close>
      <el-form label-width="90px">
        <el-form-item label="授权类型">
          <el-tag :type="typeTagMap[editDialog.type]" size="small">{{ editDialog.typeLabel }}</el-tag>
        </el-form-item>
...
function handleReset() { searchForm.keyword = ''; searchForm.appId = ''; searchForm.status = ''; handleSearch() }
function canEditTargetType(type: string) { return editableTargetTypes.has(type) }
...
async function submitEditTarget() {
  ...
  try {
    const { data } = await axios.put(`/api/user-panel/licenses/${editDialog.id}/target`, { target }, {
      headers: { Authorization: `Bearer ${getToken()}` }
    })
    if (data.code === 200) {
...
async function resetLicenseKey() {
  ...
  try {
    const { data } = await axios.post(`/api/user-panel/licenses/${editDialog.id}/refresh-key`, {}, {
      headers: { Authorization: `Bearer ${getToken()}` }
    })
    if (data.code === 200) {
...
async function submitRedeem() {
  ...
  try {
    const { data } = await axios.post('/api/user-panel/cards/redeem', { cardCode }, {
      headers: { Authorization: `Bearer ${getToken()}` }
    })
    if (data.code !== 200) {
