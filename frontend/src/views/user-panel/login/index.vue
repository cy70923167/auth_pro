<template>
  <div class="license-home">
        ...
        <div class="page-shell hero-grid">
          <div class="hero-content">
            ...
            <p class="hero-description">
              {{ siteSubtitle }}。从授权开通、状态查询到在线续费，为软件服务提供完整、可靠的授权管理体验。
            </p>
                ...
                <div class="query-panel">
                  ...
                  <div v-if="licenseQuerySearched" class="query-results">
                    <div v-if="licenseQueryList.length" class="result-summary">
                      <div>
                        <span class="result-icon"><IconifyIcon icon="ri:checkbox-circle-fill" /></span>
                        <div>
                          <strong>查询完成</strong>
                    ...
                    <div v-if="licenseQueryList.length" class="license-result-grid">
                      <article v-for="(item, index) in licenseQueryList" :key="`${item.appName}-${item.openedAt}-${index}`" class="license-result-card">
                        <div class="result-card-head">
                          <div class="result-app-icon"><IconifyIcon icon="ri:shield-keyhole-line" /></div>
                  ...
                  <div v-if="agentQuerySearched" class="query-results">
                    <div v-if="agentQueryResult" class="license-result-grid">
                      <article class="license-result-card">
                        <div class="result-card-head">
                          <div class="result-app-icon"><IconifyIcon icon="ri:user-star-line" /></div>
                          <div class="result-app-name">
        ...
        <div class="page-shell">
          ...
          <div class="feature-grid">
            <article v-for="feature in featureItems" :key="feature.title" class="feature-card">
              ...
              <div class="feature-link">{{ feature.tag }} <IconifyIcon icon="ri:arrow-right-up-line" /></div>
            </article>
          </div>
        </div>
      </section>
      ...
      <section id="workflow" class="workflow-section section-block">
        <div class="page-shell workflow-shell">
          <div class="section-heading workflow-heading">
            ...
            <el-button type="primary" size="large" @click="openAuthDialog(registrationEnabled ? 'register' : 'login')">
      ...
      <section class="cta-section">
        <div class="page-shell cta-card">
          ...
          <div class="cta-actions">
            <el-button size="large" plain @click="scrollToSection('query-card')">查询授权</el-button>
            <el-button type="primary" size="large" @click="openAuthDialog('login')">立即登录</el-button>
          </div>
        </div>
      </section>
    </main>
    ...
    <footer class="site-footer">
      <div class="page-shell footer-shell">
        <div class="footer-brand">
          ...
          <div><strong>{{ siteName }}</strong><span>{{ siteSubtitle }}</span></div>
        </div>
    ...
    <el-dialog
      ...
      :width="mode === 'register' ? 'min(640px, calc(100vw - 28px))' : 'min(480px, calc(100vw - 28px))'"
      align-center
      append-to-body
    ...
    >
      <template #header>
        <div class="auth-brand">
          <img :src="resolvedLogo" alt="网站 Logo" />
          <div><strong>{{ siteName }}</strong><span>用户服务中心</span></div>
        </div>
      </template>
      ...
      <transition name="card-fade" mode="out-in">
        <div v-if="mode === 'login'" key="login" class="auth-form-wrapper">
          ...
          <el-form ref="loginFormRef" :model="loginForm" :rules="loginRules" class="auth-form" @keyup.enter="handleLogin">
            <el-form-item prop="username">
              <el-input v-model="loginForm.username" placeholder="手机号 / 邮箱 / 用户ID" size="large">
            ...
            <el-button class="auth-submit" type="primary" size="large" :loading="loading" @click="handleLogin">登录用户中心</el-button>
          </el-form>
          <div class="auth-switch">
            <template v-if="registrationEnabled">
              <span>还没有账号？</span>
              <el-link type="primary" :underline="false" @click="mode = 'register'">立即注册</el-link>
            </template>
        ...
        <div v-else-if="mode === 'register'" key="register" class="auth-form-wrapper">
          ...
          <el-form ref="registerFormRef" :model="registerForm" :rules="registerRules" class="auth-form register-form" @keyup.enter="handleRegister">
            ...
            <el-form-item prop="emailCode">
              <div class="email-code-row">
                <el-input v-model="registerForm.emailCode" placeholder="6 位邮箱验证码" size="large" maxlength="6" inputmode="numeric">
            ...
            <el-button class="auth-submit" type="primary" size="large" :loading="loading" @click="handleRegister">注册并进入用户中心</el-button>
          </el-form>
          <div class="auth-switch">
            <span>已有账号？</span>
        ...
        <div v-else key="forgot" class="auth-form-wrapper">
          ...
          <el-form ref="forgotFormRef" :model="forgotForm" :rules="forgotRules" class="auth-form" @keyup.enter="handleForgot">
            <el-form-item prop="email">
              <el-input v-model="forgotForm.email" placeholder="注册邮箱" size="large">
                <template #prefix><IconifyIcon icon="ri:mail-line" /></template>
              </el-input>
            </el-form-item>
            <el-button class="auth-submit" type="primary" size="large" :loading="loading" @click="handleForgot">发送重置链接</el-button>
          </el-form>
          <div class="auth-switch">
            <span>想起密码了？</span>
            <el-link type="primary" :underline="false" @click="mode = 'login'">返回登录</el-link>
    ...
    >
      <div class="email-captcha-content">
        <p>请拖动滑块完成验证，验证通过后将向 {{ registerForm.email.trim() || '注册邮箱' }} 发送验证码。</p>
  ...
  registerFormRef.value?.validate(async (valid: boolean) => {
    ...
    try {
      ...
      if (data.code === 200) {
        ...
        const { data: loginData } = await axios.post('/api/user-panel/login', { account: email, password })
...
.window-dots {
  ...
  i {
    ...
    &:first-child { background: #ff6b6b; }
    &:nth-child(2) { background: #f7b84b; }
    &:last-child { background: #4ecb71; }
  }
}
...
.preview-title {
  ...
  small { margin-top: 3px; font-size: 10px; color: var(--el-text-color-placeholder); }
}
...
.preview-grid {
  ...
  small { font-size: 10px; color: var(--el-text-color-placeholder); }
  span { margin-top: 4px; font-size: 12px; font-weight: 600; }
}
...
.activity-row {
  ...
  i { width: 6px; height: 6px; background: var(--el-color-success); border-radius: 50%; }
...
.query-security-tip {
  ...
  svg { flex: 0 0 auto; width: 15px; height: 15px; }
}
...
.result-summary {
  ...
  small { margin-top: 2px; font-size: 11px; color: var(--el-text-color-placeholder); }
}
...
.result-app-name {
  ...
  strong { font-size: 14px; }
  span { margin-top: 2px; font-size: 11px; color: var(--el-text-color-placeholder); }
}
...
.status-badge {
  ...
  &.is-active { color: var(--el-color-success); background: rgb(103 194 58 / 10%); }
  &.is-expired { color: var(--el-color-warning); background: rgb(230 162 60 / 10%); }
  &.is-revoked { color: var(--el-color-danger); background: rgb(245 108 108 / 10%); }
}
...
.result-meta-grid {
  ...
  small { font-size: 10px; color: var(--el-text-color-placeholder); }
  strong { margin-top: 5px; overflow: hidden; font-size: 12px; text-overflow: ellipsis; white-space: nowrap; }
}
...
.query-empty {
  ...
  p { margin: 7px 0 15px; font-size: 12px; color: var(--el-text-color-placeholder); }
}
...
.feature-card {
  ...
  h3 { margin: 19px 0 10px; font-size: 18px; }
  p { min-height: 66px; font-size: 13px; line-height: 1.75; color: var(--el-text-color-secondary); }
}
...
.workflow-item {
  ...
  p { margin: 0; font-size: 12px; line-height: 1.6; color: var(--el-text-color-secondary); }
}
...
.cta-card {
  ...
  h2 { margin: 8px 0 8px; font-size: 31px; color: #fff; }
  p { margin: 0; font-size: 14px; color: rgb(255 255 255 / 72%); }
  .section-kicker { color: rgb(255 255 255 / 68%); }
}
...
.cta-actions {
  ...
  :deep(.el-button) { height: 46px; border-radius: 11px; }
  :deep(.el-button--primary) { color: var(--el-color-primary); background: #fff; border-color: #fff; }
  :deep(.el-button.is-plain) { color: #fff; background: rgb(255 255 255 / 8%); border-color: rgb(255 255 255 / 42%); }
}
...
.footer-brand {
  ...
  div { display: flex; flex-direction: column; }
  strong { font-size: 13px; }
  span { margin-top: 2px; font-size: 10px; color: var(--el-text-color-placeholder); }
}
...
.card-fade-enter-from { opacity: 0; transform: translateX(14px); }
.card-fade-leave-to { opacity: 0; transform: translateX(-14px); }
...
.auth-brand {
  ...
  img { width: 34px; height: 34px; object-fit: contain; }
  div { display: flex; flex-direction: column; }
  strong { max-width: 300px; overflow: hidden; font-size: 14px; text-overflow: ellipsis; white-space: nowrap; }
  span { margin-top: 2px; font-size: 10px; color: var(--el-text-color-placeholder); }
}
...
.email-code-row {
  ...
  :deep(.el-input) { min-width: 0; }
  :deep(.el-button) { flex: 0 0 118px; height: 46px; border-radius: 8px; }
}
...
.auth-switch {
  ...
  :deep(.el-link) { margin-left: 5px; font-size: 12px; }
}
...
@media (max-width: 860px) {
  ...
  .header-query { display: none; }
  .header-actions { min-width: 0; }
  .hero-section { padding: 126px 0 84px; }
  .hero-grid { grid-template-columns: 1fr; }
  .hero-content { text-align: center; }
  .hero-description { margin-right: auto; margin-left: auto; }
  .hero-actions, .hero-trust { justify-content: center; }
  .hero-visual { width: min(500px, 94%); margin: 10px auto 0; }
  .feature-grid { grid-template-columns: repeat(2, 1fr); }
  .workflow-shell { grid-template-columns: 1fr; gap: 44px; }
  .workflow-heading { max-width: 640px; text-align: center; margin: 0 auto; }
  .cta-card { align-items: flex-start; flex-direction: column; }
  .cta-actions { margin: 28px 0 0; }
  .result-meta-grid { grid-template-columns: repeat(2, 1fr); }
}

@media (max-width: 620px) {
  .page-shell { width: min(100% - 30px, 1180px); }
  .nav-shell { width: calc(100% - 24px); }
  .brand-logo-wrap { flex-basis: 36px; width: 36px; height: 36px; }
  .brand-logo { width: 27px; height: 27px; }
  .brand-copy strong { max-width: 118px; font-size: 14px; }
  .brand-copy small { display: none; }
  .header-actions :deep(.el-button) { padding: 8px 11px; }
  .header-actions :deep(.el-button + .el-button) { margin-left: 4px; }
  .hero-section { padding: 112px 0 70px; }
  .hero-content h1 { font-size: 32px; letter-spacing: -1px; }
  .hero-description { font-size: 14px; }
  .hero-actions { flex-direction: column; }
  .hero-actions :deep(.el-button) { width: 100%; margin-left: 0; }
  .hero-trust { display: grid; gap: 10px; text-align: left; }
  .dashboard-card { transform: none; }
  .floating-card { display: none; }
  .section-block { padding: 76px 0; }
  .section-heading { margin-bottom: 32px; }
  .section-heading h2 { font-size: 29px; }
  .flip-toggle-buttons { flex-wrap: wrap; }
  .flip-toggle-btn { flex: 1; min-width: 140px; justify-content: center; }
  .query-panel { padding: 20px 16px; border-radius: 17px; }
  .query-input-row { flex-direction: column; }
  .query-input-row :deep(.el-button) { flex-basis: 48px; width: 100%; }
  .query-security-tip { align-items: flex-start; line-height: 1.55; }
  .feature-grid { grid-template-columns: 1fr; }
  .feature-card { min-height: 0; }
  .workflow-item { grid-template-columns: 32px 40px 1fr; gap: 10px; padding: 16px; }
  .step-number { font-size: 10px; }
  .step-icon { width: 38px; height: 38px; }
  .step-icon svg { width: 18px; height: 18px; }
  .workflow-item h3 { font-size: 14px; }
  .workflow-item p { font-size: 11px; }
  .workflow-list::before { left: 40px; }
  .cta-card { padding: 36px 26px; }
  .cta-card h2 { font-size: 25px; }
  .cta-actions { flex-direction: column; margin-top: 22px; margin-left: 0; }
  .cta-actions :deep(.el-button) { width: 100%; }
  .footer-shell { flex-direction: column; gap: 18px; text-align: center; }
  .footer-meta { flex-direction: column; gap: 8px; }
  .result-meta-grid { grid-template-columns: 1fr; gap: 12px; }
  .form-grid-two { grid-template-columns: 1fr; gap: 0; }
  :global(.home-auth-dialog .el-dialog__body) { padding: 22px 20px 24px !important; }
  :global(.home-auth-dialog .el-dialog__header) { padding: 18px 20px 14px !important; }
}

@media (max-width: 430px) {
  .brand-copy strong { max-width: 105px; }
  .dashboard-body { padding: 15px; }
  .preview-grid, .result-meta-grid { grid-template-columns: 1fr 1fr; gap: 13px; }
  .result-card-head { align-items: flex-start; }
  .status-badge { margin-left: 7px; }
  .email-code-row { flex-direction: column; }
  .email-code-row :deep(.el-button) { flex-basis: 42px; width: 100%; }
}
</style>