...
func PlanList(c *gin.Context) {
	...
	query := fmt.Sprintf(`
		SELECT p.id, p.app_id, a.app_name, p.name, p.duration_days, p.price, p.sort,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.AppID, &item.AppName, &item.Name, &item.DurationDays,
...
func PlanCreate(c *gin.Context) {
	...
	result, err := db.Exec(`
		INSERT INTO license_plans (app_id, name, duration_days, price, sort, enabled, remark)
		VALUES (?, ?, ?, ?, ?, ?, ?)
	`, req.AppID, req.Name, req.DurationDays, req.Price, req.Sort, req.Enabled, req.Remark)
	if err != nil {
...
func PlanUpdate(c *gin.Context) {
	...
	_, err = db.Exec(`
		...
		SET app_id = ?, name = ?, duration_days = ?, price = ?, sort = ?, enabled = ?, remark = ?
		WHERE id = ?
	`, req.AppID, req.Name, req.DurationDays, req.Price, req.Sort, req.Enabled, req.Remark, id)
	if err != nil {
