	...
	for rows.Next() {
		...
		if err == nil {
			for planRows.Next() {
				...
				if err := planRows.Scan(&plan.ID, &plan.Name, &plan.DurationDays, &priceText); err != nil {
					continue
				}
				if priceText.Valid && priceText.String != "" {
					plan.Price, _ = strconv.ParseFloat(priceText.String, 64)
				}
...
func UserPurchase(c *gin.Context) {
	...
	`, req.PlanID, req.AppID).Scan(&appName, &planName, &cost, &durationDays)
	if err != nil {
	...
	result, err := tx.Exec(`
		...
		licenseNo, req.AppID, req.PlanID, cost, req.Type, userID,
	...
	_, err = tx.Exec(`
		...
		orderNo, userID, req.AppID, req.PlanID, userID,
		req.Type, req.Domain, formatCents(floatAmountToCents(cost)), formatCents(floatAmountToCents(cost)),
		fmt.Sprintf("用户余额支付购买授权"), licenseID, licenseNo)
	if err != nil {
