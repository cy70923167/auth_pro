...
func discountedAgentPrice(price, discount float64) float64 {
	...
	return float64(floatAmountToCents(price*discount/10)) / 100
}
...
func UserPurchaseOrderStatus(c *gin.Context) {
	...
	var status, licenseNo, payMethod, amount string
...
func userPurchaseOnline(c *gin.Context, appID int64, planID int64, licenseType string, domain string, payType string, userID uint) {
	...
	if err == nil && payConfig.validateForPay() == nil && payConfig.isPayTypeEnabled(payType) {
		...
		if err := insertAllowedLicensePurchaseOrder(db, orderNo, 0, appID, planID, "user", ownerID, licenseType, domain, cost, cost, payType, frontendReturnURL); err != nil {
...
func agentPanelPurchaseOnline(c *gin.Context, appID int64, planID int64, userID int64, licenseType string, domain string, payType string, agentID uint) {
	...
	var durationDays int
	err = db.QueryRow(`
		SELECT a.app_name, p.name, p.price, p.duration_days
		FROM license_plans p
		JOIN apps a ON a.id = p.app_id
		WHERE p.id = ? AND p.app_id = ? AND p.enabled = 1 AND a.enabled = 1
	`, planID, appID).Scan(&appName, &planName, &originalPrice, &durationDays)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"code": 400, "msg": "套餐不存在、已禁用或应用已下架"})
		return
	}
	...
	cost := discountedAgentPrice(originalPrice, discount)
	...
	if err == nil && payConfig.validateForPay() == nil && payConfig.isPayTypeEnabled(payType) {
		...
		if err := insertAllowedLicensePurchaseOrder(db, orderNo, agentID, appID, planID, ownerType, ownerID, licenseType, domain, cost, originalPrice, payType, frontendReturnURL); err != nil {
			if err == errPurchaseTypeNotAllowed {
				c.JSON(http.StatusOK, gin.H{"code": 400, "msg": purchaseLicenseTypeNotAllowedMessage(licenseType)})
			} else if err == sql.ErrNoRows {
				c.JSON(http.StatusOK, gin.H{"code": 400, "msg": "应用不存在或已下架"})
...
func insertAllowedLicensePurchaseOrder(db *sql.DB, orderNo string, agentID uint, appID int64, planID int64, ownerType string, ownerID int64, licenseType string, domain string, cost float64, originalPrice float64, payType string, returnURL string) error {
	tx, err := db.Begin()
	if err != nil {
		return err
	}
	defer tx.Rollback()
	if err := requireAppPurchaseLicenseType(tx, appID, licenseType); err != nil {
		return err
	}
	...
	if _, err := tx.Exec(`
		INSERT INTO license_purchase_orders (
			...
			type, target, amount, original_amount, pay_channel, pay_method, status, return_url, remark
		) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'easypay', ?, 'pending', ?, ?)
	`, orderNo, agentID, uid, appID, planID, ownerType, ownerID,
		licenseType, domain, formatCents(floatAmountToCents(cost)), formatCents(floatAmountToCents(originalPrice)), payType, returnURL, orderRemark); err != nil {
		return err
	}
	return tx.Commit()
}
...
func settleLicensePurchaseOrder(db *sql.DB, orderNo string, paidCents int64, gatewayTradeNo string, payMethod string, notifyPayload string) error {
	...
	err = tx.QueryRow(`
		...
		WHERE order_no = ?
		FOR UPDATE
	`, orderNo).Scan(&id, &agentID, &ownerType, &ownerID, &appID, &planID, &licenseType, &target, &amountText, &originalPrice, &status)
	if err != nil {
		return err
	}
	...
	if err != nil {
		return fmt.Errorf("套餐已不存在，无法生成授权")
	}
...
func ensureLicensePurchaseOrderSchema(db *sql.DB) error {
	if _, err := db.Exec(`
		CREATE TABLE IF NOT EXISTS license_purchase_orders (
			...
			amount DECIMAL(12,2) NOT NULL COMMENT '代理实际应付金额',
