	...
	for rows.Next() {
		...
		if err == nil {
			for planRows.Next() {
				...
				planRows.Scan(&plan.ID, &plan.Name, &plan.DurationDays, &plan.OriginalPrice)
				plan.Discount = discount
				plan.Price = discountedAgentPrice(plan.OriginalPrice, discount)
...
func AgentPanelPurchase(c *gin.Context) {
	...
	`, req.PlanID, req.AppID).Scan(&appName, &planName, &originalPrice, &durationDays)
	if err != nil {
	...
	_, err = tx.Exec(`
		...
		orderNo, agentID, req.UserID, req.AppID, req.PlanID, ownerType, ownerID,
		req.Type, req.Domain, formatCents(floatAmountToCents(cost)), formatCents(floatAmountToCents(originalPrice)),
		payMethod, orderRemark, licenseID, licenseNo)
	if err != nil {
