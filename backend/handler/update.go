...
const maxOnlineUpdatePackageSize = int64(512 << 20)
...
func AdminOnlineUpdateApply(c *gin.Context) {
	if runtime.GOOS != "linux" {
		c.JSON(http.StatusOK, gin.H{"code": 400, "msg": "一键整包更新仅支持 Linux 宝塔部署环境"})
		return
	}
...
func fetchOnlineUpdateManifest() (*onlineUpdateManifest, error) {
	...
	if err != nil || parsed.Host == "" || (parsed.Scheme != "http" && parsed.Scheme != "https") {
		return nil, errors.New("更新清单地址格式不正确")
	}
...
func fetchOnlineUpdateReleases(manifest *onlineUpdateManifest, forceRefresh bool) ([]onlineUpdateRelease, string, error) {
	...
	client := &http.Client{
		...
		CheckRedirect: func(req *http.Request, via []*http.Request) error {
			if req.URL.Scheme != parsedReleasesURL.Scheme || !strings.EqualFold(req.URL.Host, parsedReleasesURL.Host) {
				return errors.New("历史版本清单重定向到非同源地址")
			}
			if len(via) >= 5 {
				return errors.New("历史版本清单重定向次数过多")
			}
			return nil
		},
	}
...
func resolveOnlineUpdateReleasesURL(manifest *onlineUpdateManifest) (string, error) {
	manifestURL, err := url.Parse(strings.TrimSpace(config.GetUpdateManifestURL()))
	if err != nil || manifestURL.Host == "" || (manifestURL.Scheme != "http" && manifestURL.Scheme != "https") {
		return "", errors.New("更新清单地址格式不正确")
	}
...
var onlineUpdateVersionPartPattern = regexp.MustCompile(`\d+`)
...
func parseOnlineUpdateVersion(value string) ([]int, bool) {
	...
	matches := onlineUpdateVersionPartPattern.FindAllString(value, -1)
	if len(matches) == 0 {
		return nil, false
	}
