...
func (conn *purchaseLicenseTypeTestConn) QueryContext(_ context.Context, query string, _ []driver.NamedValue) (driver.Rows, error) {
	conn.state.mu.Lock()
	...
	case strings.Contains(query, "FROM license_purchase_orders") && strings.Contains(query, "FOR UPDATE"):
		return &purchaseLicenseTypeTestRows{
			columns: []string{"id", "agent_id", "owner_type", "owner_id", "app_id", "plan_id", "type", "target", "amount", "original_amount", "status"},
			values:  [][]driver.Value{{int64(1), int64(7), "user", int64(42), int64(9), int64(3), "key", "", "10.00", "20.00", "pending"}},
		}, nil
	...
	case strings.Contains(query, "SELECT CASE") && strings.Contains(query, "FROM agents a"):
		return &purchaseLicenseTypeTestRows{
			...
			values:  [][]driver.Value{{float64(10)}},
		}, nil
...
func TestDisabledTypeDoesNotCreateOnlinePurchaseOrder(t *testing.T) {
	...
	err := insertAllowedLicensePurchaseOrder(
		db, "LP-test", 2, 9, 3, "agent", 2, "key", "", 10, 20, "alipay", "https://example.test/return",
	)
...
func TestAdminLicenseCreationIgnoresPurchaseTypeMask(t *testing.T) {
	...
	body := []byte(`{"appId":9,"type":"key","ownerType":"user","ownerId":7,"remark":"manual"}`)
