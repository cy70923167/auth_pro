...
CREATE TABLE `licenses` (
  ...
  `source`         ENUM('admin','agent','user_purchase') NOT NULL COMMENT '来源: admin后台开通/agent代理开码/user_purchase用户购买',
  ...
  `max_domains`    INT UNSIGNED DEFAULT 1 COMMENT '最大可绑定域名/IP数量',
  `remark`         VARCHAR(255) DEFAULT '' COMMENT '备注',
  `created_at`     DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at`     DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `license_domains` (
  ...
  `domain`      VARCHAR(255) NOT NULL COMMENT '绑定的域名或IP地址',
...
CREATE TABLE `transactions` (
  ...
  `type`          ENUM('recharge','consume','refund','purchase') NOT NULL COMMENT '类型: recharge充值/consume消费扣费/refund退款/purchase用户购买',
